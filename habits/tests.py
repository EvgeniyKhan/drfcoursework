...
class HabitTests(APITestCase):
    ...
    def test_habit_detail(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.get(reverse('habits:view_habit', kwargs={'pk': self.habit.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertContains(response, self.habit.place)
    ...
    def test_habit_update(self):
        ...
        response = self.client.patch(reverse('habits:update_habit', kwargs={'pk': self.habit.pk}), data=data,
                                     format='json')
    ...
    def test_reward_and_pleasant_habit(self):
        ...
        response = self.client.post(reverse('habits:create_habit'), data=data, format='json')
